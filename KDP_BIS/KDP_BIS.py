...
def BIS_from_snowflake():
    session = getConnection()
    BIS_df = getTable(session)
...
def create_new_row(row):
    ...
    new_row = {'Version': 1, 'Model': 7, 'Neg': False, 'Base file condition': '', 'Pop Base file': '',
       'Starting position': 0, 'Length.1': 0, 'Valid from': '2003181 05:15:00', 'Valid to': '9999525 23:59:59',
       'Plass.week from': 190001, 'To plass.week': 210001, 'Shift in': '', 'Shift out': ''}
    for col_name in KDP_df.columns: # Iterate through original KDP columns to check mapping
        if col_name in KDP_to_BIS:
        # Access the value from the current row being processed by apply
            col_value = row[col_name]
        # print(col_value)
            if col_value in SW_partNumOrder:
                new_row[KDP_to_BIS[col_name]] = SW_partNumOrder[col_value]
            elif col_name == 'ECU':
...
if __name__ == "__main__":
    output_columns = ['Item', 'Item Type', 'Version', 'Model', 'No', 'Type', 'Text',
    ...
    SW_partNumOrder = {'SWS1': 27, 'SWE1': 39, 'SWLM': 75, 'SWL2': 84, 'SWL3': 93, 'SWL4': 102, 'SWP2': 138, 'SWM1': 30,
    ...
    print(temp.head(20))
    new_rows_list = temp.apply(lambda row: create_new_row(row), axis=1).tolist()
